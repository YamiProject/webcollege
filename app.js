...
app.get("/mygroup",async(req,res)=>{
    if(typeof req.session.user=='undefined' || req.session.user=='none'){
        ...
        res.render("c_mygroup",{studentsList:group[0],title:group[1][0].spetiality_abbreviated});
    }
});
//Страница студента
app.get("/student/:id",(req,res)=>{
    if(typeof req.session.user=='undefined' || req.session.user=='none'){
        ...
        if(serverUser.getUserGroups()==data[1]){
            res.render("c_student", {student:student});
        }
        else{
...
app.get("/student/:id/documents",(req,res)=>{
    if(typeof req.session.user=='undefined' || req.session.user=='none'){
        ...
        let studentsDocumentary=serverUser.createQuery(`SELECT * \
        FROM students \ 
        WHERE student_id=${JSON.stringify(req.params.id).replace(/\"/gi,'')};
        SELECT *\
        FROM passports\
        ...
        FROM documents\
        WHERE student_id=${JSON.stringify(req.params.id).replace(/\"/gi,'')};`)
        res.render("c_student_info",{student:studentsDocumentary[0],passport:studentsDocumentary[1],documents:studentsDocumentary[2]});
    }
});
...
app.get("/student/:id/portfolio",(req,res)=>{
    if(typeof req.session.user=='undefined' || req.session.user=='none'){
        ...
        FROM portfolio\
        WHERE student_id=${JSON.stringify(req.params.id).replace(/\"/gi,'')};`)
        res.render("c_student_portfolio",{student:studentPortfolio[0],portfolio:studentPortfolio[1]});
    }
});
